...
const SelectedButton = ({ textView, isOpenList, filterBy, listSelect, openAndClose, selectedSingle, styles, id, selectId }: Props) => {
  return (
    <div className="relative" id={id}>
      {/* <p className="my-1 textThemeOne">{textView}</p> */}
      <button
        className={`${styles ? "selectedButtonBase widthAndHeight1" : "selectedButtonBase widthAndHeight1"} ${
          isOpenList ? "rounded-b-0" : "rounded-b-[12px]"
        } py-4 px-4 bg-white rounded-t-[12px] text-[14px] font-bold text-[#A9AEB4] w-full mb-4 mt-2`}
          ...
          {listSelect.map((item, index) => (
            ...
            >
              <div className="fontSize1">
                <span className="font-bold text-[#554D77] text-[14px] ml-3 block">{item}</span>
              </div>
