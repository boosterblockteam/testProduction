...
import { DataChart, DataChartMock, dataServidorGraphMock } from "./dataChartMock";
...
export default function LinealChart() {
    ...
    if (allGraphic) {
      ...
      const getLastDays = (numberOfDays: number) => {
        ...
        for (let i = 0; i < numberOfDays; i++) {
          let day = new Date(date.getFullYear(), date.getMonth(), date.getDate() - i);
          days.unshift(`${day.getFullYear()}-${("0" + (day.getMonth() + 1)).slice(-2)}-${("0" + day.getDate()).slice(-2)}`);
        }
      ...
      const giveOneDay = () => {
        ...
        //@ts-ignore
        const lastTwoDays = allGraphic[selectCurrentGraphic].filter((item: any) => {
        ...
        function getLastTwentyFourHours() {
          ...
          for (let i = 0; i < 24; i++) {
            let hour = new Date(date.getFullYear(), date.getMonth(), date.getDate(), date.getHours() - i);
        ...
        //@ts-ignore
        option.xAxis.data = getLastTwentyFourHours().map((hour) => hour + ":00");
      ...
      const giveMeOneWeek = () => {
        //@ts-ignore
        option.xAxis.data = getLastDays(7).map((day) => dayNames[new Date(day).getDay()] + "-" + day.split("-")[2]);
      ...
      const giveMeOneMonth = () => {
        //@ts-ignore
        option.xAxis.data = getLastDays(30).map((day) => monthNames[Number(day.split("-")[1]) - 1] + "-" + day.split("-")[2]);
        ...
        option.series[0].data = getLastTwelveMonths().map((month) => {
          ...
          return allGraphic[selectCurrentGraphic]
            .filter((item: any) => {
              ...
              const monthText = monthIndex + 1 < 10 ? `0${monthIndex + 1}` : `${monthIndex + 1}`;
  ...
  const classButton = "btn-primary text-white bg-[#AD98FF] focus:ring-4 focus:ring-blue-300 font-medium rounded-[10px] text-[14px] px-3 py-2 focus:outline-none";

  return (
    <div className="px-1 rounded-[16px] w-full">
        ...
        <div className={`text-[#A9AEB4] text-[14px] font-medium  relative`}>
          ...
          >
            <div className="flex justify-between items-center w-full">
              <p>{isSelected}</p>
              <svg width="8" height="5" viewBox="0 0 8 5" fill="currentColor" xmlns="http://www.w3.org/2000/svg" className='stroke="text-black hover:text-indigo-100"'>
                <path d="M1 1L4 4L7 1" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
              </svg>
            </div>
          </button>
          {isOpen ? (
      ...
      <div className="mx-auto">
        <div className="overflow-auto">
          <div id="main" style={{ width: "500px", height: "140px" }}></div>
        </div>
        {/* <div id="main" style={{ position: "relative", height:"300px", overflow:"hidden", width: "100vw"}}></div> */}

        <div className=" flex align-center justify-center">
          <div className="px-4 py-1 rounded-[8px] bg-[#F2F3F8] mt-4">
