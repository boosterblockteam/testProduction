...
function validatePassword(value: string): string | undefined {
  ...
  if (value.length > MAX_CHARACTER || value.length < MIN_CHARACTER) {
    return "The password value must be less than 30 and greater than 8";
  }
}

function validateUrl(value: string): string | undefined {
  let MAX_CHARACTER = 200;
  let MIN_CHARACTER = 4;
  ...
  if (value.length > MAX_CHARACTER || value.length < MIN_CHARACTER) {
    return "The url value must be less than 200 and greater than 4";
  }

  try {
    const url = new URL(value);
    const hostnameSplit = url.hostname.split(".")
    if (hostnameSplit.length < 2 || !hostnameSplit?.[1]) {
      return "Url format is not valid, remember to include .com, .net or any other domain";
    }
  } catch (err) {
    if (!value.includes("http://") || !value.includes("https://")) {
      return "Url format is not valid, remember to include http:// or https:// at the beginning";
    }
